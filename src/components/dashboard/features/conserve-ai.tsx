...
export function ConservAI({ user }: ConservAIProps) {
  ...
  const generateChartData = (text: string) => {
    // Extract numerical data from text using regex
    const years = text.match(/\b(19|20)\d{2}\b/g) || [];
    const numbers = text.match(/\b\d+(?:,\d+)*(?:\.\d+)?\b/g) || [];
    
    if (years.length > 0 && numbers.length > 0) {
      return years.slice(0, 5).map((year, index) => ({
        year,
        value: parseFloat(numbers[index]?.replace(/,/g, '') || '0'),
      }));
    }
    return null;
  };
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const chartData = generateChartData(response.text);
              ...
              {messages.map((message) => (
                ...
                >
                  ...
                  >
                    ...
                    {message.data?.type === 'chart' && message.data.values && (
